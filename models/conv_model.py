...
class SimpleConvNet(nn.Module):
  def __init__(self, in_channels = 1, layer1channels = 16, layer2channels = 16, out_size = 1, in_size = 193):
    ...
    self.fc = nn.Linear(in_features = layer3dim*layer3dim*layer2channels, out_features = out_size)
  def forward(self, x):
    out = self.layer1(x)
    out = self.layer2(out)
    out = self.layer3(out)
