...
params = {'batch_size': 200, # low for testing
...
n = len(well_labels)
shuffled_indexes = np.random.randint(low = 0, high = n, size = n)
train_set = shuffled_indexes[0:200]
test_set = shuffled_indexes[200:300]
...
initial_train_set = OrganoidDataset(path2files = path, well_labels = [well_labels[i] for i in train_set], day_label_X = [day_label_X[i] for i in train_set], Y = [finalSizes for i in train_set], intensity_mean = day1_mean_and_var['mean'][0], intensity_var = day1_mean_and_var['variance'][0])
training_generator = data.DataLoader(initial_train_set, **params)

test_params = {'batch_size': 100, 'shuffle': True, 'num_workers' : 1}

initial_test_set = OrganoidDataset(path2files = path, well_labels = [well_labels[i] for i in test_set], day_label_X = [day_label_X[i] for i in test_set], Y = [finalSizes[i] for i in test_set], intensity_mean = day1_mean_and_var['mean'][0], intensity_var = day1_mean_and_var['variance'][0])
test_generator = data.DataLoader(initial_test_set, **test_params)
...
for epoch in range(max_epochs):
  ...
  for local_X, local_Y in training_generator:
    ...
    avgMSE = avgMSE + train_error
  train_error_array[epoch] = avgMSE
  test_error = 0.0
  for local_X, local_Y in test_generator:
    model.eval()
    Y_hat = model.forward(local_X)
    test_error = loss(Y_hat, local_Y).item()
  test_error_array[epoch] = test_error
  np.savetxt(fname = "train_error_200image_training_lr0.0001.txt", X = train_error_array[range(epoch + 1)])
  np.savetxt(fname = "test_error_200image_training_lr0.0001.txt", X = test_error_array[range(epoch + 1)])
