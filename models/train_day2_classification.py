...
day2_mean_and_var = pandas.read_csv('day2_mean_and_var.txt', sep = '\t', header = 0)

initial_train_set = OrganoidDataset(path2files = path, well_labels = well_labels, day_label_X = day_label_X, Y = nonEmptyWellIndicator, intensity_mean = day2_mean_and_var['mean'][0], intensity_var = day2_mean_and_var['variance'][0])
...
for epoch in range(max_epochs):
  ...
  np.savetxt(fname = "../LogisticResults/full_image_day2_avg_err_lr0.001_kernel3_64channels.txt", X = avg_error_array[range(epoch + 1)])
  np.savetxt(fname = "../LogisticResults/full_image_day2_batch_err_lr0.001_kernel3_64channels.txt", X = batch_error_array[range(epoch + 1)])
